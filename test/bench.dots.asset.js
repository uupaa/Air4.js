 ...
 */
        if (!node.jump) {
/*keep
            node.jump = Easing.jump(node.jump || {}, 24, node.y);
            clock.on(_jumpTick);
 */
/*
            node.jump = Easing.jump2(node.jump || {}, 24, node.y,
                                    function tick() {
                                        node.y = Easing.jump2(node.jump)["y"];
                                    }, function done() {
...
/*
            node.jump = Easing.jump2(node.jump || {}, 24, node.y,
                                    function tick(y) {
                                        node.y = y;//Easing.jump2(node.jump)["y"];
                                    }, function done() {
                                        node.command("NEXT_CUE");
                                    });
 */

            var jump = new Jump3(24, node.y, function(y, cy, iy) {
                                if (cy <= 0) {
                                    node.y = iy;
                                    node.command("NEXT_CUE");
                                    return true; // end
...
/*
    function _jumpTick() {
        var y = Easing.jump(node.jump)["y"];

        if (node.jump.cy <= 0) { // finished
            clock.off(_jumpTick);
            node.y = node.jump.iy;
            node.jump.ing = false;
            node.command("NEXT_CUE");
